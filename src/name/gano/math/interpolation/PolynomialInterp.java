...
public class PolynomialInterp
{
    ...
    public static double polint(double[] xa, double[] ya, double x)
    {
        ...
        for (int i = 1; i <= n; i++) //Here we find the index ns of the closest table entry, 
        {
            ...
            c[i - 1] = ya[i - 1]; //and initialize the tableau of cs and ds.
            d[i - 1] = ya[i - 1];
        }
        ...
        { //For each column of the tableau,
            for (int i = 1; i <= n - m; i++)
            { //we loop over the current cs and ds and update them. 
                ho = xa[i - 1] - x;
                hp = xa[i + m - 1] - x;
                w = c[i + 1 - 1] - d[i - 1];

                // This error can occur only if two input xs are (to within roundoff) identical.
                if ((den = ho - hp) == 0.0)
                {
                ...
                den = w / den;
                d[i - 1] = hp * den;// Here the cs and ds are updated.
                c[i - 1] = ho * den;
            }
        ...
        linear route through the tableau to its apex, updating ns accordingly to keep track of
